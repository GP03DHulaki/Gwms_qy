...
function clearSearchKey(type){
	if(type==1){
		...
		if(param=="0"){
			textClear('list','sk_voucherid,start_date,end_date,sk_expressname,sk_invcode,sk_districtname,sk_customername,b_date,e_date,ismore,fqty,oqty');
			if($('edit:sk_flag')){
		...
		textClear('list','sk_voucherid,start_date,end_date,sk_flag,sk_expressname,sk_barcode,sk_districtname,sk_customername,b_date,e_date,ismore,fqty,oqty');
	}
}
...
function ShowLock(baco,whid,qty){
 	if(parseInt(qty)!='0'){
 		Gwallwin.winShowmask("TRUE");
 		showModal('showlock.jsf?baco='+baco+'&whid='+whid+'&time='+new Date(),720,500);
		Gwallwin.winShowmask("FALSE");
 	}
}