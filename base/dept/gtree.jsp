...
<%
	...
	String sql = "SELECT id,dpid,'(' + dpid + ')' + dpna As dpna,'#' as wind,upid,stat FROM dept " + 
		"where (upid =? And ? = '') Or (upid = ? And dpid = ? Or dpid = ?)";
    ...
    try {
        ...
        while (rs.next()) {                 			//遍历结果集创建item节点
			...
			xmlstring = xmlstring + rs.getString("dpid");   
			...
			if (rs.getString("stat").toString().equals("0")){
				...
				String isSpreadSql="select 1 from dept where upid =?";         
				v = new Vector();
			...
			xmlstring = xmlstring + rs.getString("dpna");
            xmlstring = xmlstring + "\">";
            xmlstring = xmlstring + rs.getString("dpna");
