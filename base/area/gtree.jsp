...
<%
	...
	String sql = "SELECT id,geid,'(' + geid + ')' + gena As gena,'#' as wind,upid,stat FROM gein " + 
		"where (upid =? And ? = '') Or (upid = ? And geid = ? Or geid = ?)";
    ...
    try {
        ...
        while (rs.next()) {                 			//遍历结果集创建item节点
			...
			xmlstring = xmlstring + rs.getString("geid");   
			...
			if (rs.getString("stat").toString().equals("0")){
				...
				String isSpreadSql="select 1 from gein where upid =?";         
				v = new Vector<String>();
				
				v.add(rs.getString("geid"));
			...
			xmlstring = xmlstring + rs.getString("gena");
            xmlstring = xmlstring + "\">";
            xmlstring = xmlstring + rs.getString("gena");
