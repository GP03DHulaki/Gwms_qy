...
function formsubmit(){
	...
	function clearText(){
		textClear('edit','ids,maco,mana,rema,isva,wanu','N');
		//$('edit:isym').value='0';
	...
	//选择耗材编码
	function selectMaco(){
		showModal("../../common/inve/inve.jsf?retid=edit:maco&retname=edit:mana");
	}
	...
	//验证form
	function formCheck(){
		var maco = document.getElementById("edit:maco");
		if(maco==null || maco.value.Trim().length<=0){
			Gwin.alert("系统提示","材料编码不能为空！","!",document);
			$("edit:maco").focus();
			return false;
		}else{
			if(existzh_CN(maco.value)){
