function EditCustomer(cuid){
		showModal('cuwh.jsf?cuid='+cuid,990,450);
	}
...
function formsubmit(){
	...
	function addDiv(){
		clearText();
		$("edit:cuid").disabled="";//"禁用".
		$("edit:updateflag").value = "ADD";
		showDiv("新增客户");
	}

	function clearText(){
		textClear('edit','cuid,cuna,cecu,phon,adde,emai,rema,cuty,cous,cori,orid,tyna,gena,lico,erpc','N');
		$('edit:stat').value='1';
		//$('edit:isym').value='0';
	...
	function showDiv(title){
		...
		if(title.indexOf("新增")!=-1)Gwin.setLoadok("customerWin");
	}
	...
	//验证form
	function formCheck(){
		var cuid = document.getElementById("edit:cuid");
		if(cuid==null || cuid.value.Trim().length<=0){
			Gwin.alert("系统提示","客户编码不能为空！","!",document);
			$("edit:cuid").focus();
			return false;
		}else{
			if(existzh_CN(cuid.value)){
		...
		var cuna = document.getElementById("edit:cuna");
			if(cuna==null || cuna.value.Trim().length<=0){
			Gwin.alert("系统提示","客户名称不能为空！","!",document);
			$("edit:cuna").focus();
			return false;
		}
		/*
