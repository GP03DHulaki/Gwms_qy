...
<%
	...
	ScmInveMB scmInveMB = (ScmInveMB)MBUtil.getManageBean("#{scmInveMB}");
	//scmInveMB.init
%>
...
function add() {
	showModal('selectcolor.jsf?retid=edit:colo&retname=edit:cona',"","450px",parent.document,document.GwinID);
}
function del(id) {
	document.getElementById("mmain_free").removeChild(document.getElementById(id));
...
function save(){
	parent.document.getElementById("edit:colorlist").value = hcolodata.innerHTML;
	Gwin.close(document.GwinID);
}
function init(){
	var list = parent.document.getElementById("edit:colorlist").value;
	...
	for(var i=0;i<list.length;i++) {
		...
		if(colo!=""){
			data += "<span class='element' id="+colo[0]+">["+colo[0]+"]"+colo[1]+"<img src='delete.gif' title='删除' onclick='del("+colo[0]+")'/></span>"
		}
	}
