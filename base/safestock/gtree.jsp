...
<%
	...
	String sql = "SELECT id,whid,'(' + whid + ')' + whna As whna,'#' as wind,pwid,stat FROM waho " + 
		"where (pwid =? And ? = '') Or (pwid = ? And whid = ? Or whid = ?)";
    ...
    try {
        ...
        while (rs.next()) {                 			//遍历结果集创建item节点
			...
			xmlstring = xmlstring + rs.getString("whid");   
			xmlstring = xmlstring + "\" id = \"" + rs.getString("id") + "\" stat=\"" + rs.getString("stat");
			xmlstring = xmlstring + "\" itemtype=\"";
			if (rs.getString("stat")!=null&&rs.getString("stat").toString().equals("0")){
				xmlstring = xmlstring + "S";
			}else{
				String isSpreadSql="select 1 from waho where pwid =?";         
				v = new Vector();
				
				v.add(rs.getString("whid"));
			...
			xmlstring = xmlstring + rs.getString("whna");
            xmlstring = xmlstring + "\">";
            xmlstring = xmlstring + rs.getString("whna");
