			...
			<f:view>
				<h:form id="edit">
					<div id=mmain_opt>
						...
						<a4j:commandButton type="button" value="重置" onclick="clearDetailText();"
							onmouseover="this.className='a4j_over1'"
							onmouseout="this.className='a4j_buton1'" styleClass="a4j_but1" />
					</div>
					<div id=mmain_cnd>
					...
					<div id=mmain_free>
						<a4j:outputPanel id="output">
							<g:GTable gid="gtable" gtype="grid" gversion="2"
								gselectsql="SELECT a.soco,a.inco,a.qty AS cpqty,ISNULL(b.arqty ,0) AS arqty,
									a.qty-ISNULL(b.arqty ,0) AS upqty,c.inna,c.inse,c.colo,'0' as qty
									FROM  nofd a
									LEFT JOIN (
										 SELECT a.soco,a.inco,SUM(a.qty) AS arqty
										 FROM rgde a
										 JOIN rgma b ON  a.biid = b.biid
										 WHERE b.soco='#{purchaseArrvicMB.mbean.soco}' 
										 GROUP BY a.soco,a.inco
									) b ON a.soco = b.soco AND a.inco = b.inco
									JOIN inve c ON a.inco=c.inco WHERE a.biid='#{purchaseArrvicMB.mbean.soco}' 
									AND a.soco+a.inco NOT IN (
										SELECT soco+inco FROM rgde WHERE biid='#{purchaseArrvicMB.mbean.biid}') 
									#{purchaseArrvicMB.sql} 
								"
								gwidth="550" gpage="(pagesize = -1)" gdebug="false" gsort="upqty" gsortmethod="DESC"
								gcolumn="gcid = soco(headtext = selall,name = soco,width = 20,align = center,type = checkbox ,headtype = checkbox);
									gcid = 0(headtext = 序号,name = rowid,wi	dth = 30,headtype = text,align = center,type = text,datatype=string);
									gcid = soco(headtext = 采购订单,name = soco,width = 120,headtype = sort,align = left,type = text,datatype=string);
									...
									gcid = cpqty(headtext = 预约数量,name = cpqty,width = 70,headtype = sort,align = right,type = text,datatype=number,dataformat={0});
									gcid = arqty(headtext = 已到货数量,name = arqty,width = 70,headtype = sort,align = right,type = text,datatype=number,dataformat={0});
									gcid = qty(headtext = 本次到货数量,name = qty,width = 70,headtype = hidden,align = right,type = input,datatype=number,dataformat={0});
									gcid = upqty(headtext = 未到货数量,name = upqty,width = 70,headtype = sort,align = right,type = text,datatype=number,dataformat={0});
							" />
						</a4j:outputPanel>
					</div>
