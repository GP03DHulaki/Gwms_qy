...
function endAddHead(){
	...
	if($("edit:msg")!=null && $("edit:msg").value.Trim().indexOf("添加成功")!=-1){
		window.location.href="asn_edit.jsf";
	}
...
// 打开选择商品信息页面
function selectInve(){
	showModal('../../common/inve/inve.jsf?retid=edit:inco&retname=edit:inna');
	return false;
}
...
function openAddDetail(){
	Gwallwin.winShowmask("TRUE");
	showModal('asn_adddetail.jsf',630,400);
	Gwallwin.winShowmask("FALSE");
...
function selectBar(roid){
	Gwallwin.winShowmask("TRUE");
	showModal('asn_selbar.jsf?roid='+roid);
	Gwallwin.winShowmask("FALSE");
}
...
function updateDetail(){
	...
	for(i=0;i<qty.length;i++){
		if(!num.test(qty[i]) || !num.test(inpa[i])){
			msg += "第"+(i+1)+"行数量格式错误!  \n";
		}
	}
	if(msg.length<=0){
...
function selectInventory(){
	showModal('../../common/inventory/selectinventory_asn.jsf?retid=list:vc_invcode');
}

function addNew(){
	window.location.href="asn_add.jsf";
}
...
function endDo(){
	...
	if(msg.indexOf("保存成功") != -1){
		alert(msg);
		window.location.href="asn_edit.jsf?pid="+$("list:vc_voucherid").value+"";
	}else{
		alert(msg);
