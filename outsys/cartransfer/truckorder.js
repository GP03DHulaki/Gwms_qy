...
function endDo(){
	...
	if(message.indexOf('打印成功')!=-1){
    	...
    	alert(message);
    	window.location.href="entruckplan_edit.jsf";
    }else{
    	alert(message);
    }
}
...
function endADD(){
	...
	if(msg.indexOf('单据添加成功')!=-1){
		Gwallwin.winShowmask("FALSE");
		window.location.href="truckorder_edit.jsf?time="+new Date().getTime();
	}
	Gwallwin.winShowmask("FALSE");
}
...
//跳转添加
function addNew(){
	window.location.href = "truckorder_add.jsf";
}
...
function selectWaho(){
	...
	showModal('../../common/waho/waho.jsf?type=1&retid=edit:whid&retname=edit:whna&orid='+orid);
	return true;
}
...
function endDeleteHead(){
	...
	alert($("edit:msg").value);
	if($("edit:msg").value.indexOf("删除成功")!=-1){
		window.location.href = "truckorder.jsf";
	}
}
...
//审核
function checkApp(){
	var flags = Gtable.getcolvalues($('edit:soty').value,'biilstate');
	if(flags.indexOf("未完成")!=-1){
		if(!confirm("装车单中存在【未完成】的订单,确定审核?")){
			return false;
		}else{
			Gwallwin.winShowmask("TRUE");
			return true;
...
function selectFromId(obj){
	var arr=Gtable.getselcolvalues(obj,'loco');
	if(arr.length>0){		    	
		$('edit:sellist').value=arr;
		startDo();
		return true;
...
function onRowClick(biid){
	$('edit:dbiid').value=biid;
	$('iframe_detail').contentWindow.startDo();
	$('edit:hidbut').click();
}
function loadFrame(){
	$('iframe_detail').contentWindow.location.reload();
}
...
function refBill(id){
	...
	if("有差异"==val){
		$(id).value="刷新单据";
		$(id).innerHTML="刷新单据";
		$(id).className="gcolstyle";
	}
}
function rerefBill(id){
	var val = $(id).value;
	if("刷新单据"==val){
		$(id).innerHTML="有差异";
		$(id).value="有差异";
		$(id).className="default_span";
	}
}

function refBut(stat,biid){
	if(stat=="有差异"){
		$('edit:obid').value = biid;
		$('edit:refouib').onclick();
	}
}
...
function selectLpco(){
	...
	var url = "selectlpco.jsf?time=" + new Date().getTime();
	var returnVal = showModal(url, window, 'dialogHeight:480px;dialogWidth:460px;status:no;resizable:no;');
