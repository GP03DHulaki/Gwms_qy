					...
					<div id=mmain_free>
						<a4j:outputPanel id="output">
							<g:GTable gid="gtable" gtype="grid" gversion="2" gsort="tale"
								gselectsql="SELECT biid,a.whid,b.whna,isnull(tale,'1') as tale FROM stma a join waho b on a.whid = b.whid 
										where flag >= 11 and flag < 31 and a.orid like '#{sessionScope['orid']==''?'#':sessionScope['orid']}%' and 
								...
								gcolumn="gcid = 0(headtext = 序号,name = rowid,width = 30,headtype = text,align = center,type = text,datatype=string);
									gcid = biid(headtext = 补货任务,name = biid,width = 160,headtype = sort,align = left,type = text,datatype=string);
									gcid = whna(headtext = 补货仓库,name = whna,width = 160,headtype = sort,align = left,type = text,datatype=string);
