				...
				<h:form id="edit">
					...
					<a4j:outputPanel id="headButton">
						<div id=mmain_opt>
							...
							<a4j:commandButton type="button" value="打印单据"
								...
								action="#{truckOrderMB.printReport}"
								rendered="#{truckOrderMB.PRN}" styleClass="a4j_but1"
								onclick="printReport();" oncomplete="endPrintReport();"
								reRender="hidArea,msg,outTable,output" requestDelay="1000" />
						<gw:GAjaxButton id="otpDBut" value="导出EXCEL" theme="b_theme"
								onclick="reportExcel('OUTORDER')" type="button" reRender="output,OUTORDER" />
						</div>
					</a4j:outputPanel>
					<div id=mmain_cnd>
							...
							<table cellpadding="3" cellspacing="0" border="0">
								<tbody>
									<tr>
										...
										<td>
											<h:selectOneMenu id="soty" value="#{truckOrderMB.mbean.soty}"
										...
										<td>
											<h:selectOneMenu id="orid" value="#{truckOrderMB.mbean.orid}"
												disabled="true">
									...
									<tr>
										...
										<td>
											<h:selectOneMenu id="lpco" style="width:130px;"
												value="#{truckOrderMB.mbean.lpco}" disabled="true">
												<f:selectItem itemLabel="" itemValue="" />
												<f:selectItems value="#{carrierMB.list}" />
											</h:selectOneMenu>
										</td>
										<h:panelGroup
											rendered="#{truckOrderMB.mbean.soty=='OUTORDER' ? false : true}">
											<td>
												路&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;线:
											</td>
											<td>
												<h:selectOneMenu id="lico" style="width:130px;"
													value="#{truckOrderMB.mbean.lico}" disabled="true">
										...
										<h:panelGroup
											rendered="#{truckOrderMB.mbean.soty=='OUTORDER' ? true : false}">
											<td>
												快递单号:
											</td>
											<td>
												<h:inputText id="loco" styleClass="inputtext" size="20"
													disabled="#{!truckOrderMB.commitStatus}"
													value="#{truckOrderMB.mbean.loco}" />
											</td>
										</h:panelGroup>
										<td>
											经手人:
					...
					<div id=mmain_opt>
						<a4j:outputPanel id="mainBut" rendered="true">
							<a4j:commandButton value="添加订单"
								onclick="selOutOrder('#{truckOrderMB.mbean.orid}','#{truckOrderMB.mbean.soty}');"
								oncomplete="$('edit:refBut').onclick();" styleClass="a4j_but1"
								rendered="#{truckOrderMB.ADD && truckOrderMB.commitStatus}"
								onmouseover="this.className='a4j_over1'"
								onmouseout="this.className='a4j_buton1'" />

							<a4j:commandButton value="删除订单"
								onclick="if(!selectFromId('#{truckOrderMB.mbean.soty}')){return false};"
								action="#{truckOrderMB.deleteDetail}"
								reRender="hidArea,#{truckOrderMB.mbean.soty},outbiid"
								rendered="#{truckOrderMB.MOD && truckOrderMB.commitStatus}"
								onmouseover="this.className='a4j_over1'"
								oncomplete="endDelOrder();loadFrame();"
								onmouseout="this.className='a4j_buton1'" styleClass="a4j_but1">
								<a4j:actionparam value="%" assignTo="#{truckOrderMB.dbiid}" />
							</a4j:commandButton>
						</a4j:outputPanel>
					</div>
					<a4j:outputPanel id="OUTORDER"
						...
						<g:GTable gid="OUTORDER" gtype="grid" gversion="2" gdebug="false"
							...
							gselectsql="
							SELECT a.obid,a.flag,b.cuid,b.flag AS srcflag,
							...
							FROM lobd a LEFT JOIN #{truckOrderMB.mtable} b ON a.obid=b.biid
							LEFT JOIN cuin c ON b.cuid=c.cuid LEFT JOIN noin e on b.biid=e.biid
							LEFT JOIN (SELECT DISTINCT biid FROM slst WHERE flag='1' AND buty='#{truckOrderMB.mbean.soty}') 
							d ON a.obid=d.biid WHERE a.biid= '#{truckOrderMB.mbean.biid}' "
							gpage="(pagesize = 5)"
							gcolumn="gcid = obid(headtext = selall,name = selall,width = 20,headtype = #{truckOrderMB.commitStatus ? 'checkbox' : 'hidden'},align = center,type = checkbox,datatype=string);
							gcid = 0(headtext = 行号,name = rowid,width = 30,headtype = text,align = center,type = text,datatype=string);
							gcid = obid(headtext = 出库订单,name = obid,width = 150,headtype = sort,align = left,type = text,datatype=string,gstyle=gcolstyle,gscript={onclick=onRowClick('gcolumn[obid]')});
					...
					<a4j:outputPanel id="TRANPLAN"
						rendered="#{truckOrderMB.mbean.soty=='TRANPLAN' ? true : false }">
					...
					<a4j:outputPanel id="PURCHASERETURNPLAN"
						rendered="#{truckOrderMB.mbean.soty=='PURCHASERETURNPLAN' ? true : false }">
					...
					<div style="display: none;">
						<a4j:outputPanel id="hidArea">
							...
							<h:inputHidden id="filename" value="#{truckOrderMB.fileName}" />
							<a4j:commandButton id="refBut" value="刷新列表" style="display:none;"
								oncomplete="loadFrame();"
								reRender="#{truckOrderMB.mbean.soty},headmain,hidArea,outbiid">
								<a4j:actionparam value="%" assignTo="#{truckOrderMB.dbiid}" />
							</a4j:commandButton>
							<h:inputHidden value="#{truckOrderMB.dbiid}" id="dbiid" />
							<h:inputHidden value="#{truckOrderMB.dbean.obid}" id="obid" />
							...
							<a4j:commandButton id="refouib"
								...
								action="#{truckOrderMB.updateDetail}" />
							<a4j:jsFunction name="refTable"
								reRender="frameDetail,#{truckOrderMB.mbean.soty}" />
						</a4j:outputPanel>
					</div>
					<div>
						<p></p>
								...
								<tr>
									<td>
										<a4j:outputPanel id="outbiid">装车单明细(<h:outputText
												id="outdbiid"
												value="#{truckOrderMB.dbiid=='%'?'全部':truckOrderMB.dbiid}" />):
									</a4j:outputPanel>
									</td>
								</tr>
							</table>
