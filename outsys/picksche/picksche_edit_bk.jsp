					...
					<div id=mmain_cnd>
						...
						<a4j:outputPanel id="output">
							<g:GTable gid="gtable2" gtype="grid" gversion="2" gdebug="false"
								...
								'TEST0001' AS vc_invcode,'演示物料001' AS nv_invname,'演示物料规格01' AS vc_invstandard,30 AS dc_qty
								FROM temp
								UNION ALL 
								SELECT id_id,'C-005-09B17' AS vc_warehouseid,'CODETEST002' AS vc_barcode,
								'TEST0001' AS vc_invcode,'演示物料001' AS nv_invname,'演示物料规格01' AS vc_invstandard,70 AS dc_qty
								FROM temp
								UNION ALL 
								SELECT id_id,'C-010-02B12' AS vc_warehouseid,'CODETEST003' AS vc_barcode,
								'TEST0002' AS vc_invcode,'演示物料002' AS nv_invname,'演示物料规格02' AS vc_invstandard,120 AS dc_qty
								FROM temp
								UNION ALL 
								SELECT id_id,'C-022-05A14' AS vc_warehouseid,'CODETEST004' AS vc_barcode,
								'TEST0003' AS vc_invcode,'演示物料003' AS nv_invname,'演示物料规格03' AS vc_invstandard,80 AS dc_qty
								FROM temp"
							...
							gcid = vc_invcode(headtext = 物料编码,name = vc_invcode,width = 120,headtype = text,align = left,type = text,datatype=string);
							gcid = nv_invname(headtext = 物料名称,name = nv_invname,width = 160,headtype = text,align = left,type = text,datatype=string);
							...
							gcid = vc_barcode(headtext = 物料条码,name = vc_barcode,width = 160,headtype = hidden,align = left,type = text,datatype=string);
						" />
							<!-- gpage="(pagesize = 20)" -->
						</a4j:outputPanel>
					</div>
