...
function addNew(){
	window.location.href="oqc_add.jsf";
}
...
function endDo(){
	...
	if(msg.indexOf('单据添加成功')!=-1){
		Gwallwin.winShowmask("FALSE");
		window.location.href="oqc_edit.jsf?time="+new Date().getTime();
	}
	Gwallwin.winShowmask("FALSE");
}
...
function clearSearchKey(type){
	if(type==1){
		...
		if(param=="0"){
			textClear('edit','sk_voucherid,start_date,end_date,sk_flag,sk_fomvoucherid,vc_barcode,whid');
...
function selectFromNo(){
	valuechanger();
	var url = "selectOrder.jsf?moid="+$('edit:sotyvalue').value+"&main="+$('edit:maintable').value+"&time=" + new Date().getTime();
	var returnVal = showModal(url, window, 'dialogHeight:480px;dialogWidth:460px;status:no;resizable:no;');
	//if (typeof(returnVal) != "undefined") {
		//$("edit:soco").value = returnVal;
	//}
}
...
function endDo2(){
	...
	if(msg.value.indexOf('删除成功')!=-1){
		window.location.href="oqc.jsf?isAll=0&time="+new Date().getTime();
	}
}
...
function selectCode(){
    ...
    */
    ...
    if(batp == '04'){
    	showModal('../../common/inve/inve.jsf?retid=edit:baco',660,250);
    }else if(batp == '03'){
...
function endAddDetail(){
	...
	parent.$('orders').contentWindow.refTable();
	SetCwinHeight('ifoqcdetail');
	Gwallwin.winShowmask("FALSE");
}
