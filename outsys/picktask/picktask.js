...
// 查看拣货路径
 function showPath(){
 	...
 	showModal("showPath.jsf",'620','550');
 ...
 //修改任务执行人
  function modifysius(){
 	Gwallwin.winShowmask("TRUE");
 		showModal("modifysius.jsf",'620','550');
 	Gwallwin.winShowmask("false");
 }
function doSearch(){
 Gwallwin.winShowmask("TRUE");
...
// 清除明细表中数量为0的数据
function refrechQty(){
	if(confirm("确定将当前备货数量超出可用库存数量的商品刷新为可用库存么？")){
		Gwallwin.winShowmask("TRUE");
		return true;
	}else{
		return false;
...
// 结束清除明细
function endRefrechQty(){
	alert($("edit:msg").value);
	Gwallwin.winShowmask("FALSE");
}

function addNew(){
	window.location.href="picktask_add.jsf?isAll=0";
}
...
function endAddHead(){
	...
	if($("edit:msg")!=null && $("edit:msg").value.Trim().indexOf("添加成功")!=-1){
		window.location.href="picktask_edit.jsf";
	}
	Gwallwin.winShowmask("FALSE");
}
...
function showAddDetail(){
	Gwallwin.winShowmask("TRUE");
	showModal("addDetail.jsf",'835','550');
	Gwallwin.winShowmask("FALSE");
}
...
function selectModifyUser(){
	showModal('user.jsf?retid=edit:sius1&retname=edit:sina1',475,500);
...
function selectBar(roid){
	Gwallwin.winShowmask("TRUE");
	showModal('picktask_selbar.jsf?roid='+roid);
	Gwallwin.winShowmask("FALSE");
}
...
function selAllBar(){
	Gwallwin.winShowmask("TRUE");
	showModal("picktask_selbar.jsf?roid=");
	Gwallwin.winShowmask("FALSE");
}
...
function endAddDetails(){
	...
	alert(msg);
	Gwallwin.winShowmask("FALSE");
	parent.window.document.getElementById("edit:showRe").onclick();
	window.close();
...
function endDeleteHead(){
	...
	Gwallwin.winShowmask("FALSE");
	if($("edit:msg").value.indexOf("删除成功")!=-1){
		window.location.href = "picktask.jsf";
	}
}

//保存单据
function updateHead(){
	 $("edit:siun").value = $("edit:sina").value ;
	//alert($("edit:siun").value);
	return addHead();
}
//结束保存单据
