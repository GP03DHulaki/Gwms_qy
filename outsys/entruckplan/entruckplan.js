...
function showAdjust(){
	showModal('adjustOrga.jsf','500','200');
}

function checkOrga(){

	var oorid = $("edit:oorid").value;
	var norid = $("edit:norid").value;
	if(norid=''||norid.Trim().length<=0){
		alert("变更后的组织架构不能为空");
		$("edit:norid").focus();
		return false;
	}
	if(norid==oorid){
		alert("变更后的组织架构跟原有的组织架构相同！");
		$("edit:norid").value='';
		$("edit:norid").focus();
		return false;
	}
...
function initData(){
	textClear('edit','sk_biid,start_date,end_date,sk_flag,sk_soco,orid');
}
...
function selOutOrder(orid,moid){
	showModal('selectOrder.jsf?orid='+orid,'400px','250px');
	return false;
}
...
function endDeleteHead(){
	...
	alert($("edit:msg").value);
	if($("edit:msg").value.indexOf("删除成功")!=-1){
		window.location.href = "entruckplan.jsf";
	}
}
...
function selectFromId1(obj){
	//var gid = $('edit:soty').value;

	var arr=Gtable.getselcolvalues(obj,'oubi');
	if(arr.length>0){		    	
		$('edit:sellist').value=arr;
		startDo();
		return true;
