...
//跳转添加
function addNew(){
	window.location.href = "changeowid_add.jsf";
}
...
function endHeadAdd(){
	...
	if($("edit:msg").value.indexOf("成功")!=-1){
		window.location.href="changeowid_edit.jsf";
	}else{
		Gwallwin.winShowmask("FALSE");
		return true;
	}
...
// 打开选择库位页面(明细表页面)
function selectWaho1(owid){
	showModal('waho.jsf?pwid='+owid);
	return false;
}
...
function beforechange(){
	var owid = $("edit:frow").value;
	showModal('beforechangeowid.jsf?retid=edit:baco&owid='+owid,'550','500');
	return false;
}
...
function afterchange(){
	...
	showModal('afterchangeowid.jsf?retid=edit:obco&pwid1='+baco,'720','500');
	return false;
}
...
function selectowid()
{
	showModal("selectowid.jsf?&retid=edit:frow&retname=","580px","380px");
}
function selectowid2()
{
	showModal("selectowid.jsf?&retid=edit:toow&retname=","580px","380px");
}
...
// 打开选择条码
function selectCode(owid){
	showModal('beforechangebaco.jsf?retid='+owid,600,550);
	return true;
}
function selectuser()
{
