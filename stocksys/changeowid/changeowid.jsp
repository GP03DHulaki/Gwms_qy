...
<html>
	...
	<%
	    ChangeOwidMB ai = (ChangeOwidMB) MBUtil
	            .getManageBean("#{changeOwidMB}");
	...
	<body id="mmain_body">
		<div id="mmain_nav">
			<font color="#000000">库内处理&gt;&gt;</font><b>货主调拨</b>
			<br>
		</div>
		<f:view>
			<div id="mmain">
				<h:form id="edit">
					<div id="mmain_opt">
						<a4j:commandButton value="添加单据"
							...
							oncomplete="addNew();" action="#{changeOwidMB.clearMBean}"
							id="addHead" rendered="#{changeOwidMB.ADD}" />
						<a4j:commandButton onmouseover="this.className='a4j_over1'"
							onmouseout="this.className='a4j_buton1'" styleClass="a4j_but1"
							id="deleteId" value="删除单据" type="button"
							onclick="if(!deleteHeadAll()){return false};"
							rendered="#{changeOwidMB.DEL}" reRender="output,renderArea"
						...
						<a4j:commandButton onmouseover="this.className='a4j_over'"
							...
							action="#{changeOwidMB.search}" requestDelay="50"
							onclick="startDo();" rendered="#{changeOwidMB.LST}" />
						<a4j:commandButton value="重置"
							onmouseover="this.className='a4j_over'"
							onmouseout="this.className='a4j_buton'" styleClass="a4j_but"
							onclick="clearSearchKey('0');" rendered="#{changeOwidMB.LST}" />
					...
					<div id=mmain_cnd>
						...
						<h:inputText id="sk_start_date" styleClass="datetime" size="10"
							...
							value="#{changeOwidMB.sk_start_date}"
							onfocus="#{gmanage.datePicker}" />
						<h:outputText value="至:">
						</h:outputText>
						<h:inputText id="sk_end_date" styleClass="datetime" size="10"
							onkeypress="formsubmit(event);"
							value="#{changeOwidMB.sk_end_date}"
							onfocus="#{gmanage.datePicker}" />
						条码变更单号:
						<h:inputText id="sk_biid" styleClass="datetime"
							onkeypress="formsubmit(event);" size="12"
							value="#{changeOwidMB.sk_obj.biid}" />
						制单人:
						<h:inputText id="sk_crna" styleClass="datetime"
							onkeypress="formsubmit(event);" size="12"
							value="#{changeOwidMB.sk_obj.crna}" />
						单据状态:
						<h:selectOneMenu id="sk_flag" value="#{changeOwidMB.sk_obj.flag}"
							rendered="true" onchange="doSearch();">
							<f:selectItem itemLabel="全部" itemValue="" />
							<f:selectItems value="#{changeOwidMB.flags}" />
						</h:selectOneMenu>
						组织架构:
						<h:selectOneMenu id="orid" value="#{changeOwidMB.orid}"
							onchange="doSearch();">
					...
					<a4j:outputPanel id="output">
						<g:GTable gid="gtable" gtype="grid"
							gselectsql="select b.whna,a.id,a.orid,g.orna,a.biid,a.crus,a.crna,a.crdt,a.edna,a.eddt,a.opna,a.chna,a.chdt,a.whid,a.flag,a.rema,a.frow,a.toow
										from ocma a left join waho b on a.whid = b.whid 
										left join orga g on g.orid = a.orid
										where 1=1 #{changeOwidMB.searchSQL}
							"
							...
							gcolumn="gcid = biid(headtext = selall,name = pk,width = 20,align = center,type = checkbox,headtype = checkbox);
								...
								gcid = biid(headtext = 货主变更单号,name = hid_biid,width = 0,align = left,type = text,headtype = hidden,datatype = string);
								gcid = biid(headtext = 货主变更单号,name = biid,width = 100,headtype = sort,align = left,type = link,datatype=string,type = link,linktype=script,typevalue=changeowid_edit.jsf?pid=gcolumn[biid]);
					...
					<div style="display: none;">
						<a4j:outputPanel id="renderArea">
							...
							<h:inputHidden id="biid" value="#{changeOwidMB.mbean.biid}" />
