...
function endDo(){
	...
	if(message.indexOf('打印成功')!=-1){
    	...
    	alert(message);
    	window.location.href="cancelback_edit.jsf";
    }else{
    	alert(message);
    }
}
...
//跳转添加
function addNew(){
	window.location.href = "cancelback_add.jsf";
}
...
function addHead(){
	if($("edit:owid")==null || $("edit:owid").value.Trim().length<=0){
		alert("移动库位不能为空!");
		return false;
	}
	Gwallwin.winShowmask("TRUE");
...
function delDetail(obj){
	var arr = Gtable.getselcolvalues(obj,"did");
	if(arr.length>0){		    	
...
function endDeleteHead(){
	...
	alert($("edit:msg").value);
	if($("edit:msg").value.indexOf("删除成功")!=-1){
		window.location.href = "cancelback.jsf";
	}else if($("edit:msg").value.indexOf("<br>")!=-1){
		var msgs = $("edit:msg").value.split('<br>');
		var msg = "";
		for(i = 0;i < msgs.length;i++){
...
function initEdit(){
	textClear('edit','soco,baco,qty','Y');
}
