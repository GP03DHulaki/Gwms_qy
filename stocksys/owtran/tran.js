...
function selectintysapid(){
	showModal('../../common/SAPWaho/SAPWaho.jsf?retid=edit:outwhna&retname=edit:pfwh');
	return false;
} 

//打开SAP仓库编码页面--
function selectinty(){
	var outwhna = $("edit:outwhna").value;
	
	showModal('../../common/SAPWaho/toowSAPWaho.jsf?retid=edit:powh&retname=edit:inwhna&outwhna='+outwhna);
	$("edit:outwhna1").value= $("edit:pfwh").value;
	return false;
} 
...
function selectWaho1(retid,retname,reifib){
	showModal('sap.jsf?type=1&retid='+retid+'&retname='+retname+'&reifib='+reifib+'&pwid=all');
...
function Edit(vc_voucherid,frow,toow){
	if($("edit:biid")!=null){
		$("edit:biid").value=vc_voucherid;
	}
	if($("edit:frow")!=null){
		$("edit:frow").value=frow;
	}
	if($("edit:toow")!=null){
		$("edit:toow").value=toow;
	}
	$("edit:editbut").click();
}
...
function endAddDetails(){
	...
	alert(msg);
	Gwallwin.winShowmask("FALSE");
	var showTable = parent.window.document.getElementById("edit:showRe");
