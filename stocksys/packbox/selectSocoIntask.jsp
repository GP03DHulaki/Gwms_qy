...
<%@page import="com.gwall.view.PoinMB"%><%@ include
					...
					<div id=mmain_free>
						<a4j:outputPanel id="output">
							<g:GTable gid="gtable" gtype="grid" gversion="2"
								gselectsql="select distinct a.biid,soco,convert(varchar(20),crdt,120) as crdt from inpk a where 1=1 and (a.stat=0 or a.stat=1) #{poinMB.sql} "
								...
								gcolumn="gcid = 0(headtext = 序号,name = rowid,width = 30,headtype = text,align = center,type = text,datatype=string);
									gcid = biid(headtext = 入库任务,name = biid,width = 120,headtype = sort,align = left,type = text,datatype=string);
									gcid = soco(headtext = 采购通知单,name = soco,width = 120,headtype = sort,align = left,type = text,datatype=string);
									gcid = crdt(headtext = 创建日期,name = crdt,width = 150,headtype = sort,align = left,type = text,datatype=string);
							" />
						</a4j:outputPanel>
					</div>
