...
<html>
	...
	%>
	<body id=mmain_body onload="clearwhao();">
		<div id=mmain>
			<f:view>
				<h:form id="edit">
					<div id=mmain_opt>
						<a4j:commandButton onmouseover="this.className='a4j_over1'"
							...
							id="sid" value="查询" type="button" action="#{changeBacoMB.searchwhao}"
							reRender="output" requestDelay="50" />
						<a4j:commandButton type="button" value="重置" onclick="clearwhao();"
					...
					<div id=mmain_cnd>
						...
						<h:inputText id="id" styleClass="inputtextedit" size="15"
							value="#{changeBacoMB.id}" onkeypress="formsubmit(event);" />
						<h:outputText value="库位名称:">
						</h:outputText>
						<h:inputText id="name" styleClass="inputtextedit" size="15"
							value="#{changeBacoMB.na}" onkeypress="formsubmit(event);" />
					</div>
					<div id=mmain_free>
						<a4j:outputPanel id="output">
							<g:GTable gid="gtable" gtype="grid" gversion="2" gsort="whid" gsortmethod="ASC"
								gselectsql="SELECT whid,whna FROM waho WHERE 1=1 AND 
									whfl='P' AND orid like '#{sessionScope['orid']==''?'#':sessionScope['orid']}%' 
									AND whty = '#{changeBacoMB.type}' AND pwid like '#{changeBacoMB.pwid}%'
									#{changeBacoMB.sql} "
