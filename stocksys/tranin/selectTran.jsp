...
<html>
	...
	<%
	    ...
	    ValueBinding binding = context.getApplication().createValueBinding(
	            "#{traninMB}");
	    TraninMB ai = (TraninMB) binding.getValue(context);
				...
				<h:form id="edit">
					<div id=mmain_opt>
						<a4j:commandButton onmouseover="this.className='a4j_over1'"
							...
							id="sid" value="查询" type="button" action="#{traninMB.seasql}"
					...
					<div id=mmain_cnd>
						<h:outputText value="调拨计划单号:">
						</h:outputText>
						<h:inputText id="id" styleClass="inputtextedit" size="15"
							value="#{traninMB.soco}" onkeypress="formsubmit(event);" />
					</div>
					<div id=mmain_free>
						<a4j:outputPanel id="output">
							<g:GTable gid="gtable" gtype="grid" gversion="2"
								gselectsql="select id,biid,pfwh,powh from ppma where flag ='11' OR flag='21' and biid in (select biid from gettraninbiid()) #{traninMB.sql} "
								gwidth="550" gpage="(pagesize = 20)" gdebug="true"
								growclick="selectThis('gcolumn[biid]','gcolumn[powh]')"
								gcolumn="gcid = 0(headtext = 序号,name = rowid,width = 30,headtype = text,align = center,type = text,datatype=string);
									gcid = biid(headtext = 调拨计划单号,name = biid,width = 400,headtype = sort,align = left,type = text,datatype=string);
							" />
						</a4j:outputPanel>
					</div>
					<div id="hiddenDiv" style="display: none;">
						<h:inputHidden id="retid" value="#{traninMB.retid}" />
					</div>
				</h:form>
			</f:view>
		</div>
