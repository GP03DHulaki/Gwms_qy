var Tree = new function() {
    ...
    this.createItemHTML = function(itemId, itemType, itemData, itemflag,parentitemId) {
        ...
        if (itemType == "SF" || itemType == "UF" || itemType == "HF" ||itemType == "MF") {
			retHTML = '<span class="folderMark" onclick="Tree.getChildren(\'' + itemId + '\',\'F\')" ><img height="9" src="../../images/plus.png" width="9"></span>' +
				'<input type=checkbox id=' + itemId + '_checkbox itemid=\"' + itemId + '\" itemop=\"FM\" flag=\"' + itemflag + '\" onclick="Tree.ClickCheckBox(this,\'' + itemId + '\',\'' + itemType + '\');" />' + 
        	...
        	if (itemflag == "2") {
        	 	retHTML = '<span id=' + itemId + '_span ><input type=checkbox id=' + itemId + '_checkbox checked="checked" itemid=\"' + parentitemId + '\" itemop=\"' + itemId + '\" flag=\"' + itemflag + '\" onclick="Tree.ClickCheckBox(this,\'' + itemId + '\',\'' + itemType + '\');" />'+itemData+'</span>';
			} else {
				retHTML = '<span id=' + itemId + '_span ><input type=checkbox id=' + itemId + '_checkbox itemid=\"' + parentitemId + '\" itemop=\"' + itemId + '\" flag=\"' + itemflag + '\" onclick="Tree.ClickCheckBox(this,\'' + itemId + '\',\'' + itemType + '\');" />'+itemData+'</span>';
			}
        }
        return retHTML;
    }
	...
	this.ClickCheckBox = function(obj,itemId,moduletype){	
		...
		switch(_flag){
			...
			case '0':
				...
				//为操作权限时，改变样式				if (moduletype == "HF"){
					...
					if(roleoperate==""){
						roleoperate='"'+itemId+'","HF","2"||';
					}
					else{
						roleoperate=roleoperate+'"'+itemId+'","HF","2"||';
					}
				...
				if (moduletype == "OP"){
					//obj.parentElement.className = "operatesel";
					if(roleoperate==""){
						roleoperate='"'+itemId+'","OP","2"||'
					}
					else{
						roleoperate=roleoperate+'"'+itemId+'","OP","2"||'
					}
			...
			case '2':
				...
				if (moduletype == "HF"){
					//obj.parentElement.className = "operatesel";
					if(roleoperate==""){
						roleoperate='"'+itemId+'","HF","0"||'
					}
					else{
						roleoperate=roleoperate+'"'+itemId+'","HF","0"||'
					}
				...
				if (moduletype == "OP"){
					//obj.parentElement.className = "operatesel";
					if(roleoperate==""){
						roleoperate='"'+itemId+'","OP","0"||'
					}
					else{
						roleoperate=roleoperate+'"'+itemId+'","OP","0"||'
					}
		...
		$('edit:roleoperate').value=roleoperate;
		$('edit:refbut').click();
