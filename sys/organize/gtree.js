var GTree = new function() {
    ...
    this.init = function() {
		...
		varhtml = varhtml + '<span id=\'root_span\' class="folder" itemtype = "F" onclick="GTree.clickItem(\'#\',\'root\')" >组织架构</span>';
...
//		}
	...
	this.refreshItems = function(_parentId,_childId) {
		...
		var link = null;
		if(_childrenData == null)_childrenData = []; 
	...
	this.createChildItem = function(itemobj , itemId , itemValue, itemType, itemLink) {
		...
		if (itemType == "M" ) {
			tempitem.id=itemId + "_span_img";
			tempitem.className="itemMark";
			tempitem.innerHTML = "<img height=9 src=\'../../images/page.png\' width=9>";
			itemobj.appendChild(tempitem);
			
			tempitem1 = document.createElement("span");
			tempitem1.id=itemId + "_span";
			tempitem1.className="item";
			tempitem1.setAttribute("itemtype",itemType);
			//tempitem1.setAttribute("onclick","GTree.clickItem('" + itemLink + "','" +itemId + "');");
			//tempitem1.setAttribute("ondblclick","GTree.dblclickItem('" + itemLink + "','" +itemId + "');");
			tempitem1.onclick= function () { GTree.clickItem(itemLink,itemId); } 
			tempitem1.ondblclick= function () {GTree.dblclickItem(itemLink ,itemId);}

			tempitem1.innerHTML = itemValue;
			itemobj.appendChild(tempitem1);
		}
		else if (itemType == "S") {
			tempitem.id=itemId + "_span_img";
			tempitem.className="stopMark";
			tempitem.innerHTML = "<img height=9 src=\'../../images/stop.png\' width=9>";
			itemobj.appendChild(tempitem);
			tempitem = document.createElement("span");
			tempitem.id=itemId + "_span";
			tempitem.className="stop";
			tempitem.setAttribute("itemtype",itemType);
			//tempitem.setAttribute("onclick","GTree.clickItem('" + itemLink + "','" +itemId + "');")
			//tempitem.setAttribute("ondblclick","GTree.dblclickItem('" + itemLink + "','" +itemId + "');");
			tempitem.onclick= function () { GTree.clickItem(itemLink,itemId); } 
			tempitem.ondblclick= function () {GTree.dblclickItem(itemLink ,itemId);}
			...
			itemobj.innerHTML = '';
			tempitem.id=itemId + "_span_img";
			tempitem.className="folderMark";
			tempitem.setAttribute("onclick","GTree.getChildren('" + itemId + "');");
			//tempitem.onclick= function () { GTree.getChildren(itemId); } 
			if (tmpHTML != ''){
